...
sk_sp<SkSurface> RasterBackgound::CreateSurface(int width, int height)
{
	...
	SkImageInfo info = SkImageInfo::Make(width, height, SkColorType::kBGRA_8888_SkColorType, kPremul_SkAlphaType);
	return SkSurface::MakeRasterDirect(info, pixels, sizeof(uint32_t) * width);
}
